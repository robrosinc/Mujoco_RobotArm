...
class RobotSimulator:
    ...
    def write_marker(self):
        FK_position = (f"Current :{float(self.x_current[0]):.1f}, {float(self.x_current[1]):.1f}, {float(self.x_current[2]):.1f}")
        Desired_position = (f"Desired :{float(self.x_desired[0]):.1f}, {float(self.x_desired[1]):.1f}, {float(self.x_desired[2]):.1f}")
    ...
    def move_robot_to_position(self, position):
        ...
        self.gravity = compute.gravity(self.angle, self.rows, self.cols)

        F = self.F_calcalator(self.angle)
        self.prev_error = self.error  
        J = compute.compute_jacobian(self.angle, self.rows, self.cols)
        self.tau = J.T.dot(F) - 100*(self.angle - np.array(self.q_prev)) + self.gravity
    ...
    def create_output_folder(self):
        ...
        base_folder = f"/home/robros/model_uncertainty/script/dataset_generator/csv/joint_data/new_dynamics/gravity/{current_time}"
    ...
    def run_simulation(self):
        ...
        while True: 
            ...
            # 현재 시간 측정 후 주파수 계산
            step_time = time.time()
            frequency = 1.0 / (step_time - last_step_time)
            # print(f"Step {step_count}: Frequency = {frequency:.2f} Hz")  # 주파수 출력
